...
async function main() {
    console.log("🚀 Starting deployment to Dispatch L1 Testnet...\n");

    const [deployer] = await ethers.getSigners();
    console.log("Deploying contracts with account:", deployer.address);
    console.log("Account balance:", ethers.formatEther(await ethers.provider.getBalance(deployer.address)), "AVAX\n");

    // Dispatch L1 Configuration
    const DISPATCH_L1_ADDRESSES = {
        // Note: CCIP is not currently supported on Dispatch L1
        ...
        // Native token (AVAX on Dispatch L1)
        NATIVE_TOKEN: "0x0000000000000000000000000000000000000000", // ETH equivalent on Dispatch L1

        // Chain Selectors for future CCIP integration
        DISPATCH_L1: 779672n, // Chain ID as placeholder for future CCIP selector
        AVALANCHE_FUJI: 14767482510784806043n, // Avalanche Fuji CCIP selector
        ECHO_L1: 173750n, // Echo L1 chain ID as placeholder
    };
    ...
    console.log("\n🎉 Deployment to Dispatch L1 completed successfully!\n");

    console.log("=== DISPATCH L1 DEPLOYMENT SUMMARY ===");
    console.log("Network: Dispatch L1 Testnet");
    console.log("Chain ID: 779672");
    ...
    console.log("- CCIP is not currently supported on Dispatch L1");
    ...
    console.log("- Vault Address:", vaultAddress);
    console.log("");
    console.log("🔍 Block Explorer: https://subnets-test.avax.network/dispatch");
    console.log("");
    console.log("⚠️  Next Steps:");
    console.log("1. Fund user accounts with test tokens for transfers");
    console.log("2. Update frontend configuration to include Dispatch L1 addresses");
    console.log("3. Monitor Chainlink CCIP for Dispatch L1 support");
